## 1、为什么 JavaScript 设计成单线程的?
众所周知 JavaScript 是单线程的，这是由它的用途而决定的，因为 JavaScript 需要操作 DOM，使用单线程避免并发操作 DOM 的复杂性，保证 DOM 操作安全和简化编程模型。

那么问题来了，如果既有同步代码和异步代码，单线程的 JavaScript 是如何处理的呢？假如说 JavaScript 代码在执行时遇到了一个 `setTimeout` 定时器，延时1秒，JavaScript 线程不可能自己去计时，因为它后面还有很多同步代码等待它去执行，而这时候就要依靠其它的线程了。

## 2、浏览器是多线程的

准确来讲，我们说 JavaScript 是单线程的，是指浏览器分配给 JavaScript 负责解析执行 JavaScript 脚本的线程是单线程的，而浏览器本身是多线程的，除了 JavaScript 引擎线程，还有 **GUI 渲染线程(与 JavaScript 引擎线程互斥)、事件触发线程、定时器触发线程、异步 HTTP 请求线程**等。

## 3、EventLoop 介绍
浏览器为了对任务调度进行更为精确的控制，于是将任务拆分成宏任务和微任务两类。
...
1. 执行主执行栈。
2. 清空微任务队列。
3. 执行 `requestAnimationFrame` 回调、事件处理以及刷新页面。
...
<!-- 页面渲染一次 -->
```
上述 script 整体为一个宏任务，每次宏任务执行完毕后才会进行 GUI 渲染线程的渲染，所以最终只会渲染一次，body 的背景颜色为蓝色。
```html
<script>
    document.body.style = 'background:red';
    setTimeout(function(){
...
<!-- 页面渲染两次 -->
```
script 标签和 setTimeout 都是宏任务，script 标签执行后会进行一次渲染，body 背景呈红色，而后执行 setTimeout,又会渲染一次，body 背景呈现黄色，虽然执行代码的速度很快，但还是明显能看到颜色闪烁的效果。
### 3.2 微任务
...
- 微任务: `process.nextTick（Nodejs`）, `Promise`, `Object.observe`（可以监听对象变化并执行回调的 API，目前已被废弃，被 Proxy 取代）, `MutationObserver`。
- 微任务队列会一次性清空。
```js
function loop() {
...
需要先安装下 `readline-sync` 库。`npm i readline-sync -S`。
...
// 打印结果： 先后打印四次zhangsan 
```
### 4.4 第四版 XMLHttpRequest
`XMLHttpRequest` 是由浏览器的其他进程或发起请求，然后再将执行结果利用 IPC(Inter-Process Communication)进程间通信的方式通知渲染进程，之后渲染进程再将对应的消息添加到消息队列中。
...
- libuv 是一个基于事件驱动的跨平台抽象层，封装了不同操作系统一些底层特性，对外提供统一的API。
- 事件循环机制也是它里面的实现。
...
`setTimeout/setInterval` 取值范围是[1,2的32次方-1],超出范围初始化为1，所以 `setTimeout(fn,0) === setTimeout(fn,1)`。
...
`process.nextTick` 独立于 Event Loop,有自己的队列，每个阶段完成后如果存在 nextTick 队列会全部清空，优先级高于微任务。
```js
setTimeout(() => {
...
本文主要深入介绍了浏览器和 Node.js 中的 EventLoop 的流程及原理，介绍了宏任务和微任务有哪些及其特点，以及通过具体代码模拟实现了 EventLoop，帮助大家更好的理解 EvevtLoop 原理。