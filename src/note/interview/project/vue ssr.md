...
### 占用服务端资源
- SSR：需要在客户端拼接 `HTML` 字符串，会消耗较多服务端资源。
- CSR：服务端只返回一个静态的空 `HTML`，消耗服务端资源极少。

## 用户体验
- SSR：虽然首屏速度快，但后续切换路由没有 CSR 流畅。
- CSR：切换路由并不需要真正的跳转，用户体验好。

那么，如何融合 SSR 和 CSR 的优点于一身呢？答案就是**同构渲染**。

## 同构渲染
首次渲染时，拿到HTML字符串，但与 SSR 的不同之处在于，同构渲染会在HTML字符串中拼接服务端通过API请求到的数据，这是为了后续能在客户端进行激活操作。
## hydration 激活操作
- 建立真实 DOM 与虚拟 DOM 之间的联系。
- 提取服务端序列化后的数据字符串，初始化应用状态。 

## SSR 与 CSR 和同构渲染的比较
|  | SSR | CSR | 同构渲染 |
|-------|-------|-------|-------|
| 渲染时机 | 在服务端生成完整的HTML，浏览器拿到直接渲染 | 浏览器只拿到一个空的HTML，通过执行 `JavaScript` 动态生成页面 | 在服务端生成HTML字符串，并拼接 API 接口请求数据，客户端拿到HTML字符串后，进行激活 |
| SEO友好性 | 友好 | 不友好 | 友好 |
| 首屏加载速度 | 快 | 慢 | 快 |
| 占用服务器资源 | 多 | 少 | 中 |
| 用户体验 | 差 | 好 | 好 |
...
## target: node
服务端运行的代码，不需要将其依赖的 Node.js 核心模块或第三方模块打包到最终的代码中，因为 node 环境中已经内置了，所以需要在打包配置中设置“target: node”，并借助 webpack-node-externals 插件解决第三方依赖打包的问题。
