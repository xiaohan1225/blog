import{_ as a,o as n,c as i,ae as l}from"./chunks/framework.AOaeJVPC.js";const k=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"note/interview/project/前端监控.md","filePath":"note/interview/project/前端监控.md"}'),e={name:"note/interview/project/前端监控.md"};function p(t,s,o,r,c,h){return n(),i("div",null,[...s[0]||(s[0]=[l(`<h2 id="sdk数据采集与上报核心实现思路" tabindex="-1">SDK数据采集与上报核心实现思路 <a class="header-anchor" href="#sdk数据采集与上报核心实现思路" aria-label="Permalink to &quot;SDK数据采集与上报核心实现思路&quot;">​</a></h2><blockquote><p>这里只列举了核心指标收集与监控是数据整体流程，后续如果有需要自定义事件和数据采集的，可以借助 <code>MutationObserver</code> 或 <code>Performance API</code> 进行处理。</p></blockquote><h3 id="核心模块设计" tabindex="-1">核心模块设计 <a class="header-anchor" href="#核心模块设计" aria-label="Permalink to &quot;核心模块设计&quot;">​</a></h3><ol><li>网页性能指标采集（FP、FCP、CLS等）：通过 <code>Web-vitals</code> 采集这些性能指标。</li><li>异常采集：监控 js 错误，资源加载错误和未捕获的 Promise 错误。</li><li>点击事件采集：捕获用户点击行为，记录交互路径。</li><li>基础信息采集：采集 IP、机型、系统、浏览器等设备以及环境信息。</li><li>数据传输协议：统一封装不同类别的数据，发送给后端。wo</li></ol><h3 id="核心目标" tabindex="-1">核心目标 <a class="header-anchor" href="#核心目标" aria-label="Permalink to &quot;核心目标&quot;">​</a></h3><ol><li>插件化设计：支持性能、异常、点击事件等不同采集插件。</li><li>跨环境兼容：支持浏览器和 Node 服务端的数据上报。</li><li>代码解读：数据传输逻辑与采集逻辑分离，方便维护和扩展。</li><li>Monorepo：多包管理模式，提升代码复用和管理效率。</li></ol><h3 id="sdk-核心架构设计" tabindex="-1">SDK 核心架构设计 <a class="header-anchor" href="#sdk-核心架构设计" aria-label="Permalink to &quot;SDK 核心架构设计&quot;">​</a></h3><ul><li>@xiaohan/monitor-sdk-core <ul><li>定义核心逻辑和 Transport 接口。</li><li>负责插件初始化与注册。</li></ul></li><li>@xiaohan/monitor-sdk-browser <ul><li>浏览器相关插件：性能、异常、点击事件采集。</li><li>自定义浏览器 Transport 上报实现。</li></ul></li><li>@xiaohan/monitor-browser-utils <ul><li>提供浏览器环境下的工具方法。</li></ul></li><li>@xiaohan/monitor-sdk-node <ul><li>适配 Node 环境的 Transport 上报实现。</li></ul></li><li>@xiaohan/monitor-sdk-vue/react/angular/solid【拓展】</li></ul><div class="language-plain vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">plain</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>📦monitor</span></span>
